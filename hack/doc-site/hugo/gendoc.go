...
func main() {
	...
	fmt.Println("==> Generated examples.yaml")
	...
	fmt.Println("    Visit: http://localhost:1313/examples/")
	fmt.Println()

	// Start Hugo server with both configs.
	cmd := exec.CommandContext(ctx, "hugo", "server",
		"--config", "hugo.yaml,examples.yaml",
		"--buildDrafts",
		"--disableFastRender",
		"--navigateToChanged",
		"--bind", "0.0.0.0",
		"--port", "1313",
		"--baseURL", "http://localhost:1313/examples/",
		"--appendPort=false",
		"--logLevel", "info",
		"--cleanDestinationDir",
	)
...
// generateRuntimeYAML reads the template and writes examples.yaml with substitutions.
func generateRuntimeYAML(goVersion, latestRelease, versionMessage, buildTime string) {
	tmpl, err := os.ReadFile("examples.yaml.template")
	if err != nil {
	...
	if err := os.WriteFile("examples.yaml", []byte(out), 0o600); err != nil { //nolint:mnd
		fatalf("writing examples.yaml: %v", err)
	}
}
